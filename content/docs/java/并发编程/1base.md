...
但是这里有个问题：比如i++ 这种语句是非原子性的。 甚至连 i = 10 都不是原子性的，工作线程需要在工作栈中给i赋值，然后再写会到主内存中。

### 可见性问题
由于JMM的问题，每个工作线程都会将其需要的数据拷贝一份到工作缓存中，
...
/**
 * 一个可以取消的异步调度。这个类提供了{@link Future}的一种实现，
 * 这个实现方式提供了start 和cancel的方法用于控制这个异步的计算过程，
 * 还提供了查询当前这个计算过程是否完成，以及取得计算结果的方法。
 * 这个计算结果只有当计算过程结束之后才能获取到。
 * {@code get} 这个方法将会一直block除非计算过程结束。
 * 一旦计算过程结束，那么计算过程不能被重启或者取消，
 * 除非使用{@link #runAndReset}。
 *
 * 一个 {@code FutureTask} 可以用于即实现了 {@link Callable} 也实现了
 * {@link Runnable}。一个 {@code FutureTask} 可以提交给线程池
 * {@link Executor} 来执行
 *
